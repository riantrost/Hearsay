...
- **Each place is its own clock** *(rewritten 2026-07-22 — this bullet originally read "Sessions are the clock" and described a session scrubber; both were removed as a design miss inherited from Fragments, see decisions.md "Sessions are gone")*. The campaign's time axis is not a linear session counter — tables battle at and report from the same location repeatedly, out of order, late. The record is the **history each location accumulates**: a pin's events read top to bottom as the lineage of that site, timestamps carry the ordering, and the map's illumination shows where the campaign is alive relative to its own front, never the calendar's.
