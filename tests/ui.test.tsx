...
// Component tests for the new UI — deliberately thin: the rules truth lives
...
function makeData(): CampaignData {
  return {
    campaign: { id: 'c1', name: 'Test March', mapImageUrl: '/api/maps/c1', mapW: 1600, mapH: 1200, currentSession: 2, joinCode: 'ABC123' },
    members: [
    ...
    events: [
      { id: 'e1', pinId: 'p1', session: 1, canonLine: 'The gate fell.', participantIds: [] },
      { id: 'e2', pinId: 'p1', session: 2, canonLine: 'Morning after.', participantIds: [] },
    ],
    testimony: [],
    bounties: [],
  };
...
function makeStore(data: CampaignData, viewerId = 'm2'): ApiStore {
  const store = {
    ...
    writeTestimony: vi.fn(async (eventId: string, text: string) => {
      const entry = { id: 'tX', eventId, memberId: viewerId, session: data.campaign.currentSession, text };
      data.testimony.push(entry);
      return entry;
    }),
    promoteMark: vi.fn(),
    addEvent: vi.fn(async () => ({ id: 'eX', pinId: 'p1', session: 2, canonLine: 'x', participantIds: [] })),
    addBounty: vi.fn(),
    setPinHidden: vi.fn(),
    revealPin: vi.fn(),
  };
...
describe('PinPanel — the one-box account flow', () => {
  it('an open slot is one textarea and one Save button (per event — late slots stay fillable)', () => {
    ...
    const { getAllByPlaceholderText, getAllByText } = render(<PinPanel store={store} pinId="p1" session={2} />);
  ...
  it('a half-typed draft survives a data refresh — by construction', () => {
    ...
    const { container, rerender } = render(<PinPanel store={store} pinId="p1" session={2} />);
    ...
    rerender(<PinPanel store={store} pinId="p1" session={2} />);
  ...
  it('saving writes through the store and editing is just editing (no amend mode)', async () => {
    const data = makeData();
    const store = makeStore(data);
    const { container, queryByText } = render(<PinPanel store={store} pinId="p1" session={2} />);
    // the session-2 (grace-open) slot is the second .your-account
  ...
  it('a sealed account reads locked, plainly', () => {
    const data = makeData();
    data.testimony.push({ id: 't1', eventId: 'e1', memberId: 'm2', session: 1, text: 'old words' });
    const store = makeStore(data);
    const { getByText } = render(<PinPanel store={store} pinId="p1" session={2} />);
    getByText('Saved — locked when session 2 began');
  });

  it('owner sees Add event with the who-was-there picker; player does not', () => {
    const data = makeData();
    const owner = render(<PinPanel store={makeStore(data, 'm1')} pinId="p1" session={2} />);
    owner.getByText('Add event');
    owner.getByText('Who was there');
    cleanup();
    const player = render(<PinPanel store={makeStore(data, 'm2')} pinId="p1" session={2} />);
  ...
  it('unchecking a member sends an explicit participant subset', async () => {
    ...
    const { container, getByText } = render(<PinPanel store={store} pinId="p1" session={2} />);
...
describe('BountyBoard', () => {
  it('shows the live character budget and plain group names', () => {
    const data = makeData();
    data.bounties.push({ id: 'b1', campaignId: 'c1', postedBy: 'm2', target: 'The rope-cutter', reason: 'He cut it.', session: 1, status: 'posted' });
    const store = makeStore(data);
